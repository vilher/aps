...
//get user by email
app.get("/:where/:email", function(req,res){
        con.query("SELECT * FROM "+req.params.where+" WHERE Elektroninis_pastas= '"+req.params.email+"'",function (err, result) {
          if (err) { res.send([{"Elektroninis_pastas": "error"}]);}
          else{
            if(result.length===0)res.send([{"Elektroninis_pastas": "false"}]);
            else{
            res.send(result);}
          }
        });
